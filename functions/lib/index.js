...
const webpushConfig = (0, params_1.getAppConfig)("webpush");
...
exports.getVapidPublicKey = (0, https_1.onCall)({ region: europeWest2 }, (request) => {
    ...
    if (!publicKey) {
        v2_1.logger.error("CRITICAL: VAPID public key (webpush.public_key) not set in function configuration.");
...
exports.zipProjectFiles = (0, https_1.onCall)({ region: europeWest2, timeoutSeconds: 300, memory: "1GiB" }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "You must be logged in.");
    }
    const projectId = request.data?.projectId;
    ...
    for (const doc of filesSnap.docs) {
        ...
        if (!data.fullPath || !data.name) {
            v2_1.logger.warn("Skipping file with missing data", doc.id);
            continue;
        }
        try {
            ...
            v2_1.logger.error("Download failed:", data.fullPath, err);
        }
    }
    if (added === 0) {
        throw new https_1.HttpsError("internal", "Files exist in Firestore but none could be downloaded from Storage.");
    }

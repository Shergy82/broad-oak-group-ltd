  ...
  const handleCreateProject = async (values: z.infer<typeof projectSchema>) => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'Could not create project. Check Firestore rules.',
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new project to the database. You can upload files to it after
            creation.
          </DialogDescription>
        </DialogHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Council Name" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
          ...
          async () => {
            try {
              ...
              await addDoc(collection(db, `projects/${project.id}/files`), {
                ...
                uploaderId: uid, // MUST match request.auth.uid
              ...
              console.error(
                `Failed to save file info for ${file.name} to Firestore:`,
                dbError
              );
              reject(dbError);
            }
...
function FileManagerDialog({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        setFiles(
          snapshot.docs.map((d) => ({ id: d.id, ...d.data() } as ProjectFile))
        );
        setIsLoading(false);
      },
      (error) => {
  ...
  const handleDownloadPhotosAsPdf = async () => {
    ...
    const addTitlePage = () => {
      ...
      docPdf.text('Project Photo Report', 105, 80, { align: 'center' });

      docPdf.setFontSize(16);
      const addressLines = docPdf.splitTextToSize(project.address, 180);
      docPdf.text(addressLines, 105, 100, { align: 'center' });

      docPdf.setFontSize(12);
      if (project.eNumber) {
        docPdf.text(`E-Number: ${project.eNumber}`, 105, 120, { align: 'center' });
      }
      if (project.manager) {
        docPdf.text(`Manager: ${project.manager}`, 105, 128, { align: 'center' });
      }

      docPdf.setFontSize(10);
      docPdf.setTextColor(150);
      docPdf.text(`Report generated on: ${format(new Date(), 'PPP p')}`, 105, 150, {
        align: 'center',
      });
    };
    ...
    try {
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        const response = await fetch(
          `https://images.weserv.nl/?url=${encodeURIComponent(file.url)}`
        );
        ...
        docPdf.addImage(imageData, 'JPEG', x, y, imgWidth, imgHeight);

        docPdf.setFontSize(8);
        docPdf.setTextColor(120);

        const uploadedAt = file.uploadedAt
          ? format(file.uploadedAt.toDate(), 'dd/MM/yyyy p')
          : 'Unknown date';

        const footerText = `${file.name} | Uploaded by: ${file.uploaderName} on ${uploadedAt}`;
        docPdf.text(footerText, pageWidth / 2, pageHeight - 10, { align: 'center' });
      }

      docPdf.save(`${project.address.replace(/[^a-zA-Z0-9]/g, '_')}_photos.pdf`);
  ...
  const handleDeleteFile = async (file: ProjectFile) => {
    ...
    try {
      const deleteProjectFileFn = httpsCallable<{ projectId: string; fileId: string }>(
        functions,
        'deleteProjectFile'
      );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload new files or delete existing ones for this project.
          </DialogDescription>
        </DialogHeader>
                    ...
                    {files.map((file) => (
                      <TableRow key={file.id}>
                        ...
                        <TableCell className="text-right">
                          ...
                          {(userProfile.uid === file.uploaderId ||
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    <span className="font-semibold">{file.name}</span>. This
...
export function ProjectManager({ userProfile }: ProjectManagerProps) {
  ...
  const handleDownloadPhotosAsPdf = async (project: Project) => {
    ...
    const imageFiles = filesSnapshot.docs.map((d) => d.data() as ProjectFile);
    ...
    const addTitlePage = () => {
      ...
      docPdf.setFontSize(12);
      if (project.eNumber) docPdf.text(`E-Number: ${project.eNumber}`, 105, 120, { align: 'center' });
      if (project.manager) docPdf.text(`Manager: ${project.manager}`, 105, 128, { align: 'center' });
  ...
  const handleDeleteProject = async (project: Project) => {
    ...
    if (!functions) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'Firebase Functions service is not available.',
      });
      return;
    }
    ...
    try {
      const deleteProjectAndFilesFn = httpsCallable<{ projectId: string }>(
        functions,
      ...
      toast({
        title: 'Success',
        description: 'Project and all its files have been deleted.',
      });
    } catch (error: any) {
      ...
      toast({
        ...
        description:
          error.message ||
          'An unknown error occurred. Please check the function logs in the Firebase Console.',
      });
    }
  };
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    This is a highly destructive action. To confirm, please enter
                    your password. This will permanently delete ALL projects and
                ...
                <DialogFooter>
                  ...
                  >
                    {isReauthenticating || isDeletingAll ? (
                      <Spinner />
                    ) : (
                      'Confirm & Delete All'
                    )}
                  ...
                  <TableRow key={project.id}>
                    ...
                    <TableCell>
                      {project.nextReviewDate
                        ? format(project.nextReviewDate.toDate(), 'dd/MM/yyyy')
                        : 'N/A'}
                    </TableCell>
                    <TableCell className="text-right space-x-2">
                      ...
                      {['admin', 'owner', 'manager'].includes(userProfile.role) && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleDownloadPhotosAsPdf(project)}
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                and all of its associated files. This action cannot be undone.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
              ...
              <Card key={project.id}>
                ...
                <CardContent className="text-sm space-y-2">
                  ...
                  <div>
                    ...
                    {project.createdAt ? format(project.createdAt.toDate(), 'dd/MM/yyyy') : 'N/A'}
                  </div>
                  <div>
                    <strong>Next Review:</strong>{' '}
                    {project.nextReviewDate ? format(project.nextReviewDate.toDate(), 'dd/MM/yyyy') : 'N/A'}
                  </div>
                </CardContent>

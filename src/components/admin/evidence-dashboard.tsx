...
function ProjectEvidenceCard({ project, checklist, files, loadingFiles, onMarkAsComplete, onResetStatus, onScheduleForDeletion }: ProjectEvidenceCardProps) {
    ...
    const [isGalleryPdfGenerating, setIsGalleryPdfGenerating] = useState(false);
    ...
    const handleDownloadGalleryPdf = async () => {
        if (filteredGalleryPhotos.length === 0) return;
        setIsGalleryPdfGenerating(true);
        toast({ title: 'Generating PDF...', description: 'Please wait, collecting filtered photos.' });
        ...
        try {
            ...
            const filterTitle = galleryTagFilter === 'all' ? 'Evidence Report' : `${galleryTagFilter} Evidence`;
            doc.text(filterTitle, pageWidth / 2, currentY, { align: 'center' });
            currentY += 20;
            ...
            doc.text(`Generated on: ${format(new Date(), 'PPP')}`, pageWidth / 2, pageHeight - 25, { align: 'center' });
            if (userProfile) {
                doc.text(`Generated by: ${userProfile.name}`, pageWidth / 2, pageHeight - 20, { align: 'center' });
            }
            ...
            filteredGalleryPhotos.forEach(photo => {
                const tag = photo.evidenceTag || 'Uncategorized';
                if (!groupedPhotos.has(tag)) groupedPhotos.set(tag, []);
                groupedPhotos.get(tag)!.push(photo);
            });
            const sortedGroups = new Map([...groupedPhotos.entries()].sort());
            ...
            for (const [tag, photosInGroup] of sortedGroups.entries()) {
                ...
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(255, 255, 255);
                doc.text(tag, pageMargin + titlePadding, finalY + (titleHeight + titlePadding) / 2, { align: 'left', baseline: 'middle' });
            ...
            doc.save(`evidence_${project.address.replace(/[^a-zA-Z0-9]/g, '_')}_${galleryTagFilter}.pdf`);
            toast({ title: 'Success', description: 'Filtered evidence report generated.' });
        } catch (error: any) {
            console.error("PDF generation failed:", error);
            toast({ variant: 'destructive', title: 'Error', description: 'Could not generate filtered PDF.' });
        } finally {
            setIsGalleryPdfGenerating(false);
        }
    };
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 py-2 border-b">
                        ...
                        <div className="flex items-end gap-2">
                            <Button 
                                ...
                                onClick={handleDownloadGalleryPdf} 
                                disabled={isGalleryPdfGenerating || filteredGalleryPhotos.length === 0}
                            >
                                {isGalleryPdfGenerating ? <Spinner size="sm" /> : <><Download className="mr-1 h-3 w-3" /> Download PDF</>}
                            </Button>

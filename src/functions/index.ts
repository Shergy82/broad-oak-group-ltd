...
export const getVapidPublicKey = onCall({ region: "europe-west2", cors: true }, async () => {
  if (!VAPID_PUBLIC) {
...
export const setNotificationStatus = onCall({ region: "europe-west2", cors: true }, async (req) => {
...
export const deleteAllShifts = onCall({ region: "europe-west2", cors: true }, async (req) => {
...
export const setUserStatus = onCall({ region: "europe-west2", cors: true }, async (req) => {
    if (!req.auth?.uid) throw new HttpsError("unauthenticated", "Authentication required.");
    const userDoc = await db.collection("users").doc(req.auth.uid).get();
...
export const deleteUser = onCall({ region: "europe-west2", cors: true }, async (req) => {
    if (!req.auth?.uid) throw new HttpsError("unauthenticated", "Authentication required.");
    const userDoc = await db.collection("users").doc(req.auth.uid).get();
    const userRole = userDoc.data()?.role;
    if (userRole !== 'admin' && userRole !== 'owner') {
...
export const syncUserNamesToShifts = onCall({ region: "europe-west2", cors: true }, async (req) => {
    if (!req.auth?.uid) throw new HttpsError("unauthenticated", "Authentication required.");
    const userDoc = await db.collection("users").doc(req.auth.uid).get();
    if (userDoc.data()?.role !== 'owner') {
        throw new HttpsError('permission-denied', 'You must be an owner to perform this action.');
...
export const deleteProjectFile = onCall({ region: "europe-west2", cors: true }, async (req) => {
    if (!req.auth?.uid) throw new HttpsError("unauthenticated", "Authentication required.");
    const userDoc = await db.collection("users").doc(req.auth.uid).get();
    const userRole = userDoc.data()?.role;
...
export const deleteProjectAndFiles = onCall({ region: "europe-west2", cors: true }, async (req) => {
    if (!req.auth?.uid) throw new HttpsError("unauthenticated", "Authentication required.");
    const userDoc = await db.collection("users").doc(req.auth.uid).get();
    const userRole = userDoc.data()?.role;
    if (!['owner', 'admin', 'manager'].includes(userRole)) {
...
export const deleteAllProjects = onCall({ region: "europe-west2", cors: true }, async (req) => {
    if (!req.auth?.uid) throw new HttpsError("unauthenticated", "Authentication required.");
    const userDoc = await db.collection("users").doc(req.auth.uid).get();
    if (userDoc.data()?.role !== 'owner') {
...
export const zipProjectFiles = onCall({ region: "europe-west2", cors: true, timeoutSeconds: 300 }, async (req) => {
    if (!req.auth?.uid) {

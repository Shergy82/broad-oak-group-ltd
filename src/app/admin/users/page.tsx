            ...
            <TableRow key={user.uid} className={user.status === 'suspended' ? 'bg-muted/30' : ''}>
              ...
              <TableCell>
                {canChangeRole ? (
                  ...
                  <Badge variant={user.role === 'owner' ? 'default' : user.role === 'admin' ? 'secondary' : 'outline'} className={`capitalize ${user.role === 'owner' ? 'bg-primary text-primary-foreground' : ''} ${user.role === 'manager' ? 'bg-orange-500 hover:bg-orange-600 text-white' : ''} ${user.role === 'TLO' ? 'bg-purple-500 hover:bg-purple-600 text-white' : ''}`}>
                    {user.role}
                  </Badge>
                )}
              </TableCell>

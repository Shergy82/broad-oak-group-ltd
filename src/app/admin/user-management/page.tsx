        ...
        try {
            if (context === 'unassigned' && !department) {
                toast({
                    ...
                    description: 'Please select a department to assign this user.',
                });
                setIsLoading(false);
                return;
            }
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>
                        {context === 'unassigned'
                            ? `Assign Department for ${user.name}`
                            : `Edit User: ${user.name}`}
                    </DialogTitle>
                    <DialogDescription>
                        {context === 'unassigned'
                            ? 'Select a department to move this user to the active list.'
                            : "Modify the user's role and other details."}
                    </DialogDescription>
                </DialogHeader>
...
export default function UserManagementPage() {
    ...
    useEffect(() => {
        ...
        const q = isOwner
            ...
            : query(usersCollection, where('department', '==', currentUserProfile.department));
    ...
    const { pendingUsers, unassignedUsers, activeUsers, suspendedUsers } = useMemo(() => {
        ...
        const unassigned = searchedUsers.filter(
            (u) => u.status !== 'pending-approval' && (!u.department || u.department === '')
        );
        const assignedWithDept = searchedUsers.filter(
            (u) => u.status !== 'pending-approval' && u.department && u.department !== ''
        );
        ...
        if (isOwner) {
            if (selectedDepartments.size > 0) {
                visibleAssigned = assignedWithDept.filter(
                    (u) => u.department && selectedDepartments.has(u.department)
                );
            }
        } else if (isAdmin) {
            visibleAssigned = assignedWithDept.filter(
                (u) => u.department === currentUserProfile?.department
            );
        } else {
            visibleAssigned = [];
        }
        ...
        return {
            ...
            activeUsers: visibleAssigned
                .filter((u) => u.status === 'active' || !u.status)
                .sort(sortByName),
            suspendedUsers: visibleAssigned
    ...
    const handleSetUserStatus = async (user: UserProfile, newStatus: 'active' | 'suspended') => {
        ...
        try {
            const setUserStatusFn = httpsCallable<typeof payload, { success: boolean }>(
                functions,
                'setUserStatus'
            );
    ...
    const handleDeleteUser = async (user: UserProfile) => {
        ...
        try {
            ...
            toast({
                title: 'User Deleted',
                description: 'The user and their authentication account have been permanently removed.',
            });
        } catch (error: any) {
            toast({
                variant: 'destructive',
    ...
    const handleEditUser = (
        user: UserProfile,
        context: 'unassigned' | 'default' = 'default'
    ) => {
    ...
    const renderUserList = (
        usersToRender: UserProfile[],
        type: 'pending' | 'active' | 'suspended' | 'unassigned'
    ) => {
                            ...
                            <Card key={user.uid}>
                                ...
                                <CardFooter>
                                    <Button
                                        size="sm"
                                        className="w-full"
                                        onClick={() => handleEditUser(user, 'unassigned')}
                                    >
                                        <Edit className="mr-2 h-4 w-4" />
                                        Assign Department
                                    </Button>
                                ...
                                return (
                                    <TableRow key={user.uid}>
                                        ...
                                        <TableCell className="text-right space-x-1">
                                            {type === 'pending' && (
                                                <Button size="sm" onClick={() => handleSetUserStatus(user, 'active')}>
                                            ...
                                            {type === 'active' && (
                                                <Button
                                                    size="sm"
                                                    variant="destructive"
                                                    onClick={() => handleSetUserStatus(user, 'suspended')}
                                                >
                                                ...
                                                <AlertDialogContent>
                                                    <AlertDialogHeader>
                                                        ...
                                                        <AlertDialogDescription>
                                                            This will permanently delete {user.name} and their authentication account.
                        ...
                        return (
                            <Card key={user.uid}>
                                <CardHeader>
                                    ...
                                    {user.operativeId && (
                                        <CardDescription>ID: {user.operativeId}</CardDescription>
                                    )}
                                </CardHeader>
                                <CardContent className="text-sm space-y-3">
                                    ...
                                    <div>
                                        <strong>Trade:</strong> {user.trade || 'N/A'}
                                    </div>
                ...
                <CardContent className="space-y-4">
                    ...
                    <Tabs defaultValue="pending">
                        ...
                        <TabsContent value="unassigned" className="mt-4">
                            {renderUserList(unassignedUsers, 'unassigned')}
                        </TabsContent>
                        ...
                        <TabsContent value="suspended" className="mt-4">
                            {renderUserList(suspendedUsers, 'suspended')}
                        </TabsContent>
                    </Tabs>
                </CardContent>
            </Card>

...
export function NotificationButton() {
  ...
  // If the browser truly doesn't support it, hide the button.
  ...
  const handleToggleSubscription = () => {
    ...
    console.log('BELL CLICKED', {
      ...
      isKeyLoading,
      vapidMissing,
      vapidKeyPrefix: vapidKey ? vapidKey.slice(0, 10) : null,
    });

    if (permission === 'denied') {
      console.log('Permission is denied -> opening blocked dialog');
      setBlockedDialogOpen(true);
      return;
    }

    if (vapidMissing) {
      console.error('VAPID KEY MISSING -> cannot subscribe');
      alert('VAPID key is missing in the browser build. Token cannot be created.');
      return;
    }

    if (isSubscribed) {
      console.log('About to call unsubscribe()');
      unsubscribe();
    } else {
  ...
  const getTooltipContent = () => {
    ...
    if (vapidMissing) return 'VAPID key missing (cannot subscribe)';

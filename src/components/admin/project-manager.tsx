...
function CreateProjectDialog({ open, onOpenChange, userProfile }: CreateProjectDialogProps) {
  ...
  const form = useForm<z.infer<typeof projectSchema>>({
    ...
    defaultValues: { address: '', eNumber: '', council: '', manager: '', department: '' },
  });
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleCreateProject)} className="space-y-4 py-4">
            ...
            <FormField control={form.control} name="department" render={({ field }) => (
                <FormItem><FormLabel>Department</FormLabel><FormControl><Input placeholder="e.g. Build" {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
             <DialogFooter>

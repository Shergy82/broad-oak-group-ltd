...
export class BroadOakProfile implements PlannerProfile {
  ...
  detect(workbook: ExcelJS.Workbook): boolean {
    ...
    // Look for characteristic black/dark dividers in Column A or merged header patterns
    let darkRows = 0;
    for (let r = 1; r <= 20; r++) {
        ...
        for (const { col, date } of dateCols) {
          ...
          const flush = () => {
            if (currentOperative) {
              ...
              const { task, room, description } = this.splitTask(fullTask);
  ...
  private splitTask(text: string) {
    const roomMatch = text.match(/\b(Kitchen|Bathroom|Lounge|Bedroom|Hallway|Loft|Utility|Garage)\b/i);
  ...
  private extractNamesAndTask(text: string, userMap: UserMapEntry[]) {
    ...
    // Check if any word fragments match a user
    ...
    for (let r = start; r <= Math.min(start + 10, end); r++) {
      ...
      for (let c = 6; c <= 50; c++) {
        ...
        if (val instanceof Date && !isNaN(val.getTime())) {
          cols.push({ col: c, date: new Date(Date.UTC(val.getFullYear(), val.getMonth(), val.getDate(), 12)) });
  ...
  private extractAddress(sheet: ExcelJS.Worksheet, start: number, end: number): string | null {
    for (let r = start; r <= end; r++) {
      ...
      if (text.length > 5 && /\d/.test(text) && !/ORDERING|MANAGER|TLO/i.test(text)) {
        return text;
      }
    }
    return null;

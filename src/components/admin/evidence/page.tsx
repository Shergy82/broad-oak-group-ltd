...
function ProjectEvidenceCard({ project, checklist }: { project: Project; checklist: EvidenceChecklist | undefined }) {
  ...
  return (
    <Card className={cn("hover:shadow-md transition-shadow", isComplete ? 'border-green-500/50 bg-green-500/5' : 'border-red-700/50 bg-red-700/10')}>
        ...
        {loadingFiles ? (
          ...
          <div className="space-y-1">
            {evidenceStatus.map(item => (
                <div key={item.text} className={cn("flex items-center gap-2 text-xs", item.isComplete ? "text-muted-foreground" : "font-semibold text-red-700")}>
...
export default function EvidencePage() {
  ...
  useEffect(() => {
    ...
    // This top-level query fetches all projects.
